...
function showGreeting(lang = 'en') {
    ...
    const index = ['morning', 'afternoon', 'evening', 'night'].indexOf(
        getTimeOfDay()
    );
...
async function getWeather() {
    ...
    try {
        ...
        wind.textContent = `${weatherTranslation[lang][0][0]} ${Math.floor(
            data.wind.speed
        )} ${weatherTranslation[lang][0][1]}`;
...
const volumeOnRemove = () =>
    playerVolumeButton.classList.remove('player-volume-on');
const volumeOffAdd = () =>
    playerVolumeButton.classList.add('player-volume-off');
const volumeOffRemove = () =>
    playerVolumeButton.classList.remove('player-volume-off');
let isVolume = true;
...
const translateLanguageRu = () => {
    ...
    settingName[8].textContent = 'Теги фона';
...
const translateLanguageEn = () => {
    ...
    settingName[8].textContent = 'Background tags';
...
languageButton.forEach((el) =>
    el.addEventListener('click', () => {
        const languageActiveRemove = () =>
            languageButton.forEach((el) =>
                el.classList.remove('language-button-active')
            );
        const languageActiveAdd = () =>
            el.classList.add('language-button-active');
...
select.addEventListener('change', () => {
    if (unsplash.selected) {
        bgTag.style.display = 'block';
        getLinkToImageUnsplash(randomNum);
    } else if (flickr.selected) {
        bgTag.style.display = 'block';
        getLinkToImageFlickr(randomNum);
    } else {
        bgTag.style.display = 'none';
        setBg(randomNum);
    }
...
const setTagLocalstorage = () => {
    localStorage.setItem('bgTag', bgTag.value);
    localStorage.setItem('bgTagValue', bgTagValue);
};
window.addEventListener('beforeunload', setTagLocalstorage);

const getTagLocalstorage = () => {
    if (localStorage.getItem('bgTag')) {
        bgTag.value = localStorage.getItem('bgTag');
    }
    if (localStorage.getItem('bgTagValue')) {
        bgTagValue = localStorage.getItem('bgTagValue');
    }
    switchBg();
};
window.addEventListener('DOMContentLoaded', getTagLocalstorage);

async function getLinkToImageUnsplash() {
    const url = `https://api.unsplash.com/photos/random?query=${bgTagValue.value}&client_id=gnCgwDcRPu-bAMgzvp52Lm7p-xLHPP89lY8B8YwKN7I`;
    const res = await fetch(url);
    const data = await res.json();
    const img = new Image();
...
async function getLinkToImageFlickr() {
    const url = `https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=89227889b4c0daea409061b494f237c1&tags=${bgTagValue}&sort=relevance&safe_search=for+safe&extras=url_h&format=json&nojsoncallback=1`;
    const res = await fetch(url);
    const data = await res.json();
    const img = new Image();
    let min = Math.ceil(1);
    ...
    if (data.photos.photo[randomNum].url_h) {
        img.src = data.photos.photo[randomNum].url_h;
    }
